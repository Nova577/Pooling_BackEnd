...
class ReportService extends BaseService {
    ...
    async getQuestionnaireReport(id, operator) {
        ...
        const research = await Research.findOne({where: {questionnarie_id: id}})
        if(!research) {
            throw new HttpError('NotFound', 'research not found.', 404)
        }
        const cooperaters = research.getUsers()
        const ids = cooperaters.map(cooperater => cooperater.id)
    ...
    async _resultOfQuestionnaire(id, user) {
        ...
        const participant = await Participant.findOne({where: {user_id: user}})
        if(!participant) {
            throw new Error('participant not found.')
        }
        const eassayAnswers_obj = await participant.getEassyQuestions()
        ...
        const choiceAnswers_obj = await participant.getChoiceQuestions()
    ...
    async _reportOfQuestionnaire(id) {
        ...
        const research = await Research.findOne({where: {questionnarie_id: id}})
        const participants_obj = await research.getParticipants()
        const results = participants_obj.map(async participant => this._resultOfQuestionnaire(id, participant.user_id))
